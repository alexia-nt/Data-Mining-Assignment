...
for ngram_range in ngram_ranges:
    print(f"\n------------ Testing ngram_range={ngram_range} (Decision Tree) ------------")
    ...
    param_grid = {
        ...
        'dt__ccp_alpha': [0.0, 1e-4, 1e-3, 1e-2]
    }
    ...
    # Save predictions
    pred_file = f"y_pred_dt_{'unigrams' if ngram_range==(1,1) else 'bigrams'}.npy"
    ...
    cm_file = f"confusion_matrix_dt_{'unigrams' if ngram_range==(1,1) else 'bigrams'}.npy"
    ...
    plt.title(f"Decision Tree Confusion Matrix ({'unigrams' if ngram_range==(1,1) else 'bigrams'})")
    plt.savefig(os.path.join(CONF_MATRIX_DIR, f"confusion_matrix_dt_{'unigrams' if ngram_range==(1,1) else 'bigrams'}.png"))
    plt.close()

    # Save classification report and accuracy
    file_name = f"classification_tree_{'unigrams' if ngram_range==(1,1) else 'bigrams'}_tfidf.txt"
    ...
    clf = best_pipeline.named_steps['dt']
    ...
    # Handle edge case many features are zero-importance
    top_idx = top_idx[:min(10, len(top_idx))]
